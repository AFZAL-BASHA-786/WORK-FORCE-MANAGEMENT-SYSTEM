...
const UploadProduct: React.FC = () =>{
    ...
    const handleSubmit = (event:React.FormEvent| any) => {
        ...
        if (form.checkValidity() === true) { 
          dispatch(productActions.createProductAction(product)).then((response:any)=>{
            if(!response.error){
                /* if there is no error menas product uploaded successfully
                 To upload another product make all fileds empty */
                setProduct({
                    employeename: "",
                    email: "",
                    imageUrl: "",
    ...
    return (
        <>
            <FinalNavbar/>
            <LayoutHeading heading='Upload Worker' color='text-success'/>
            <Container>
                <Row>
                    <Col xs={4}>
                        <Form noValidate validated={validated} onSubmit={handleSubmit}>
                            ...
                            </Form.Group> 
                            {
                                ...
                                <Form.Group className='mb-2'>
                                    <Form.Select onChange={e => selectSubCategory(e)} required>
