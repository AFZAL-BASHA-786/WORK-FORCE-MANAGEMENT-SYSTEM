...
export const createProduct = async (request: Request, response: Response) => {
    try{
        let {employeename, email,imageUrl,price, mobile, flat, street, city, state, country, pinCode,categoryId,subCategoryId } = request.body
        ...
        if(userObj){
            ...
            if(userObj.isAdmin){
                ...
                let product:IProduct | any = await ProductCollection.findOne({email:email});
                if(product){
